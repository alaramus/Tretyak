...
То есть элемент [20], потом (минус 1) [19], далее (уже минус 2), [17], (еще минус 3), [14], (минус 4) [10] и т.д. UPD: В качестве бонуса, можно сделать функцию генерации массива более универсальной: при вызове функции передавать в неё необходимый размер массива (20) и диапазон генерируемых случайных чисел (2, 118)
...
def generate_sequence(arr1):
    ...
    while current_index < len(arr3):              # while выполняется пока значение current_index меньше  длинне arr3
        ...
        current_index = current_index + step      # Индекс увеличивается на текущий шаг.
        step = step + 1                           # Значение шага увеличивается
    return arr2
